...
function estimateCostCents(model, tokensIn, tokensOut, cacheWriteTokens = 0, cacheReadTokens = 0) {
...
function calculateCacheSavings(cacheReadTokens, model) {
    const rates = COST_PER_1M_TOKENS[model] || { input: 100, cacheRead: 10 };
...
function generatePromptSignature(pipelineId, step, model, errorSignature) {
...
export async function callAI(opts) {
    ...
    try {
        ...
        if (safeModel.startsWith('claude')) {
            ...
            const systemParam = systemMessages.length > 0
                ? systemMessages.length === 1
                    ...
                    : systemMessages.map(text => ({ type: 'text', text })) // Array of text blocks
                : undefined;
            ...
            if (opts.cacheableBlocks && opts.cacheableBlocks.length > 0) {
                ...
                result = await anthropic.messages.create({
                    ...
                    messages: nonSystemMessages // Only non-system messages (no 'system' role!)
                });
            }
        ...
        try {
            ...
            const globalTracker = global.costTracker;
            if (globalTracker && pipelineId) {
                globalTracker.trackModelCall(step || role || 'unknown', safeModel, tokensIn, tokensOut);
            }
        }
        catch {
...
export function selectModel(role, complexity = 'medium') {
