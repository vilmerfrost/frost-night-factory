...
import { querySolution, saveSolution, generateErrorSignature, } from './hive-mind';
/**
 * Classify error from log output
 */
export function classifyError(log) {
 ...
 */
export async function applyFix(kind, ctx) {
 ...
 */
async function fixHomepage404(ctx) {
    const pagePaths = [
...
`;
    ...
    if (!fixedContent.includes('export default')) {
        ...
        if (componentMatch) {
            ...
            fixedContent = fixedContent.replace(new RegExp(`(${componentMatch[0]})`), `export default ${componentMatch[0]}`);
 ...
 */
async function fixA11YContrast(ctx) {
    const globalsPaths = [
 ...
 */
async function fixMissingEnv(ctx) {
 ...
 */
async function fixMissingDep(ctx) {
 ...
 */
async function fixTailwindConfig(ctx) {
 ...
 */
async function fixLighthouseUrl(ctx) {
 ...
 */
async function fixMissingLayout(ctx) {
    const layoutPaths = [
...
`;
    ...
    if (!fixedContent.includes('children')) {
        ...
        if (functionMatch) {
            fixedContent = fixedContent.replace(functionMatch[0], 'export default function RootLayout({ children }: { children: React.ReactNode })');
 ...
 */
async function fixBuildFailed(ctx) {
 ...
 */
async function fixImportError(ctx) {
 ...
 */
export async function withAutoFix(phaseName, maxAttempts, phaseFn, ctx) {
    const fixes = [];
