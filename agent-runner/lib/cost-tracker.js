...
export class CostTracker {
    ...
    trackModelCall(step, model, inputTokens, outputTokens) {
    ...
    endPipeline() {
        ...
        finalCost.breakdown.forEach((item) => {
            console.log(`      ${item.step}: $${item.cost.toFixed(4)} (${item.model})`);
        });
        // ✅ P2: Cost Alerts
        ...
        if (stats.totalPipelines > 10) {
            ...
            const failedRecent = recentCosts.filter(c => c.breakdown.some(b => b.step.toLowerCase().includes('failed'))).length;
    ...
    static getStatistics() {
        ...
        costs.forEach((pipeline) => {
            pipeline.breakdown.forEach((item) => {
                stepCosts.set(item.step, (stepCosts.get(item.step) || 0) + item.cost);
            });
        });
        const mostExpensiveStep = Array.from(stepCosts.entries()).sort((a, b) => b[1] - a[1])[0]?.[0] || 'N/A';
        return {
            totalSpent,
