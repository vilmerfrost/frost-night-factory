 ...
 */
export async function runE2EDebugger(failure, projectRoot, pipelineId) {
 ...
 */
function identifyRelevantFiles(failure, projectRoot) {
    const files = [];
 ...
 */
function identifyLikelyTargetFile(failure, projectRoot) {
    if (failure.url === '/' || failure.url === '') {
 ...
 */
function buildDebuggerPrompt(failure, repoMap, codeSnippets) {
 ...
 */
function parseDebuggerResponse(response, failure, relevantFiles) {
 ...
 */
export async function applyDebuggerPatch(targetFile, patch, projectRoot) {
