...
export interface EnvRequirement {
  ...
  keys: string[];
  detected: boolean;
  reason: string; // Why it was detected (import, mention, etc.)
}
...
const ENV_RECIPES: Record<string, { label: string; keys: string[]; optional?: boolean }> = {
  supabase: {
    label: 'Supabase',
 ...
 */
export function detectEnvFromCode(codeContent: string): EnvRequirement[] {
  const detected: EnvRequirement[] = [];
  ...
  for (const [provider, recipe] of Object.entries(ENV_RECIPES)) {
    ...
    if (hasImport || hasMention || hasEnvVar) {
      let reason = '';
      if (hasImport) reason = `Import detected: ${codeContent.match(importPatterns.find(p => p.test(codeContent)) || /.*/)?.[0]?.substring(0, 50)}`;
      else if (hasMention) reason = `Provider mentioned in code`;
      else if (hasEnvVar) reason = `Environment variable used`;
 ...
 */
export function detectEnvFromPlan(planContent: string): EnvRequirement[] {
  const detected: EnvRequirement[] = [];
 ...
 */
export async function detectEnvFromProject(projectPath: string): Promise<EnvDetectionResult> {
  const detectedProviders = new Map<string, EnvRequirement>();
...
export function generateEnvFile(
  ...
  detectedRequirements: EnvDetectionResult,
 ...
 */
function getDemoValue(key: string): string {
...
export function checkMissingKeys(
  detectedRequirements: EnvRequirement[],
