...
export async function runIntegrationTestSuite(config) {
    const { workspaceRoot, backendEntry, backendPort = 8001, timeout = 30000, } = config;
    const errors = [];
    const warnings = [];
 ...
 */
function mapOpenApiTypeToTs(type) {
    const typeMap = {
        string: 'string',
        integer: 'number',
        number: 'number',
 ...
 */
async function generateMockData(workspaceRoot, backendPort) {
    try {
        const openApiResponse = await fetch(`http://localhost:${backendPort}/openapi.json`);
