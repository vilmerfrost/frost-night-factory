...
import fs from 'fs';
export async function runPlaywrightTests(workspacePath, port = 3002) {
    ...
    try {
        ...
        for (const test of tests) {
            try {
                ...
                const screenshotPath = path.join(screenshotDir, `${test.name.replace(/\s+/g, '-')}.png`);
...
async function generateTestsFromRoutes(workspacePath, page, port) {
    const tests = [];
        ...
        run: async (page) => {
            ...
            const hasLowContrast = await page.evaluate(() => {
                ...
                for (const el of elements) {
                    ...
                    if ((bg.includes('255, 255, 255') && fg.includes('255, 255, 255')) ||
...
function discoverRoutes(appDir, baseRoute = '') {
    const routes = [];
