 ...
 */
async function collectTsFiles(projectRoot) {
    const map = new Map();
    async function walk(dir) {
        ...
        for (const entry of entries) {
            ...
            const info = { fullPath, relativePath, lowerKey: key };
            if (!map.has(key))
                map.set(key, []);
 ...
 */
export async function normalizeWindowsCasing(projectRoot) {
    ...
    for (const [lowerKey, infos] of fileMap.entries()) {
        ...
        for (const dup of infos.slice(1)) {
            if (dup.relativePath.toLowerCase() === lowerKey)
                continue;
            try {
 ...
 */
export async function assertNoPascalCaseFiles(projectRoot) {
    const offenders = [];
    const fileMap = await collectTsFiles(projectRoot);
    ...
    if (offenders.length > 0) {
        ...
        throw new Error(`CasingGate failed: ${offenders.length} PascalCase files found. Run normalizeWindowsCasing() first.`);
    }
 ...
 */
export function runWindowsCasingFix(repoPath) {
 ...
 */
export async function validateNoCasingIssues(repoPath) {
    try {
