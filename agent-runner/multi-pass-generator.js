 ...
 */
export async function generateWithValidation(pipelineId, step, prompt, targetFile, projectRoot, maxAttempts = 10) {
    ...
    for (attempts = 1; attempts <= maxAttempts; attempts++) {
        ...
        try {
            ...
            const cacheableBlocks = [
                { type: 'text', text: MULTI_PASS_SYSTEM_PROMPT },
                { type: 'text', text: `# Repository Map\n\n${repoMap}` },
                { type: 'text', text: `# Codebase Context\n\n${codebaseContext}` }
            ];
            ...
            if (!validation.valid) {
                ...
                if (attempts > 1) {
                    try {
                        const reflection = await generateReflection(validation.errors.join('\n'), attempts);
                ...
                currentPrompt = buildFeedbackPrompt(prompt, code, validation.errors, attempts);
                continue; // Try again
            }
            ...
            if (targetFile.endsWith('.ts') || targetFile.endsWith('.tsx')) {
                ...
                if (!astResult.complete) {
                    ...
                    currentPrompt = buildCompletenessPrompt(prompt, code, astResult.issues, attempts);
                    continue; // Try again
                }
            }
            // SUCCESS!
...
function buildFeedbackPrompt(originalPrompt, previousCode, errors, attemptNumber) {
...
function buildCompletenessPrompt(originalPrompt, previousCode, issues, attemptNumber) {
    return `${originalPrompt}
 ...
 */
async function buildCodebaseContext(projectRoot) {
    try {
