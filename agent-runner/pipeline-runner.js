...
import { performDeepResearch, generateDeepSeekCoder, runKimiQA } from '../lib/nightFactory/modelClient';
...
import { detectProjectIntent, detectTechMatrix } from '../lib/nightFactory/intentParser';
...
import { getOracle } from '../lib/nightFactory/codebaseOracle';
...
import { isFeatureEnabled } from './lib/version-manager'; // ✅ V8: Feature flags
import { PortManager } from './lib/port-manager'; // ✅ V8: Dynamic port allocation
// =============================================================================
// 🧠 INTELLIGENT FIX SYSTEMS (V6.0 - Zero Human Input)
// =============================================================================
import { runClientDetector } from '../lib/nightFactory/clientDetector';
import { runImportRewriter } from '../lib/nightFactory/importRewriter';
import { runVisualPreFlight } from '../lib/nightFactory/visualPreFlight';
import { createPipelineContext as createNewPipelineContext, recordError, scanProjectStructure, getTesterContext } from '../lib/nightFactory/pipelineContext';
import { runBatchSurgeon, shouldUseBatchSurgeon } from '../lib/nightFactory/batchSurgeon';
import { runSelfAwareValidation } from '../lib/nightFactory/selfAwareCoder';
...
import { snapshotBeforeTester, snapshotBeforeFixer, restoreFromSnapshot, cleanupSnapshots } from '../lib/nightFactory/versionControl';
import { runErrorAutopsy, detectLoop, recordErrorOccurrence as recordAutopsyError, clearErrorHistory as clearAutopsyHistory } from '../lib/nightFactory/errorAutopsy';
import { runCompilerAgent } from '../lib/nightFactory/compilerAgent';
...
import { extractTargetFiles, ErrorCategory, getFixingStrategy, autoFixPythonError } from '../lib/nightFactory/errorClassifier';
import { CircuitBreaker } from '../lib/nightFactory/circuitBreaker';
...
import { getGoldenTemplate } from '../lib/nightFactory/goldenTemplates';
...
import { logPathOperation } from '../lib/nightFactory/pathLogger';
...
function isPageTsxActuallyValid(pagePath, repoPath) {
 ...
 */
async function enforceSrcStructure(repoPath) {
 ...
 */
async function startDevServerWithVerification(repoPath, port = 3002) {
 ...
 */
function forceResetTsConfig(repoPath) {
    ...
    fs.writeFileSync(tsConfigPath, JSON.stringify(GOLDEN_TSCONFIG, null, 2), // 🆕 ADD JSON.stringify
    'utf-8');
 ...
 */
async function createPipelineWithSteps(projectSpec, name, initialPrompt) {
    try {
 ...
 */
async function createPipelineWithStepsFallback(name, initialPrompt) {
    // Create pipeline
...
async function createStep(pipelineId, stepName, status, input = {}) {
...
async function updateStep(pipelineId, stepName, status, output) {
...
const CANONICAL_STEPS = ['research', 'planner', 'coder', 'tester', 'publisher'];
async function getStep(pipelineId, stepName) {
    ...
    if (!step) {
        if (CANONICAL_STEPS.includes(stepName)) {
            console.warn(`[DB] Step ${stepName} not found for pipeline ${pipelineId}, creating new step`);
            const { data: created, error: insertError } = await supabase
                .from('pipeline_steps')
                .insert({
...
function getProjectStructure(dir, fileList = [], rootDir = dir) {
 ...
 */
function resolveImportPath(importPath, currentFile, repoPath) {
    // Hantera alias (@/)
    let targetPath = "";
...
// ------------------------------------------------------------------
async function runClonerStep(pipeline, repoPath) {
...
// ═══════════════════════════════════════════════════════════════════
async function runResearchStep(pipeline, researchType = 'technical-constraints') {
...
// ═══════════════════════════════════════════════════════════════════
async function runK2SynthesisStep(pipelineId, userVision, perplexityReports) {
...
Synthesize this into actionable technical guidance for the development team.`;
    try {
        ...
        const repoPath = getRepoPath({ id: pipelineId });
 ...
 */
async function optimizeUserRequest(rawRequest, pipelineId) {
...
async function optimizeUserPrompt(rawPrompt, pipelineId) {
    return optimizeUserRequest(rawPrompt, pipelineId);
}
async function runPlannerStep(pipeline, repoPath) {
 ...
 */
async function rewriteImportsToAlias(code, filePath) {
    ...
    rewritten = rewritten.replace(/from\s+(['"])\.\/(components|lib|app)\/(.*?)\1/g, (match, quote, folder, rest) => {
        return `from ${quote}@/${folder}/${rest}${quote}`;
    });
    // Pattern 3: Normalize default vs named imports (Button vs { Button })
    // This ensures consistency
    rewritten = rewritten.replace(/import\s+(\{[^}]+\})\s+from/g, (match, imports) => {
 ...
 */
function validateNextImports(content) {
    const errors = [];
 ...
 */
async function validateProjectStructure(localPath, pipelineId) {
 ...
 */
async function validateNoRelativeImports(localPath) {
    ...
    const tsFiles = getAllFiles(srcPath).filter(file => file.endsWith('.ts') || file.endsWith('.tsx'));
    const violations = [];
    ...
    if (violations.length > 0) {
        throw new Error(`❌ [IMPORT VALIDATOR] Found ${violations.length} relative imports:\n` +
 ...
 */
async function injectMissingDomainTypes(workspacePath, errorLog) {
    ...
    const existingTypes = missingTypes.filter(typeName => typesContent.includes(`export type ${typeName}`) ||
 ...
 */
function validateAndFixFilename(filePath) {
 ...
 */
function autoFixEnvironmentVariables(content, filename) {
    let fixed = content;
    let changes = 0;
 ...
 */
function validateReactComponentTypes(filePath) {
    if (!fs.existsSync(filePath)) {
    ...
    if (untypedChildrenPattern.test(content) && !content.includes('React.ReactNode')) {
        ...
        fixedContent = fixedContent.replace(/\{\s*children\s*\}:\s*\{[^}]*\}/g, '{ children }: { children: React.ReactNode }');
    ...
    if (functionComponentPattern.test(fixedContent) && !fixedContent.includes('interface')) {
        ...
        if (match) {
            ...
            fixedContent = fixedContent.replace(functionComponentPattern, `${interfaceDef}export default function ${componentName}({ children }: ${componentName}Props) {`);
    ...
    if (implicitAnyPattern.test(fixedContent) && !fixedContent.includes('React.ReactNode')) {
        fixedContent = fixedContent.replace(/\(\{\s*children\s*\}\s*:\s*\{[^}]*\}\)/g, '({ children }: { children: React.ReactNode })');
 ...
 */
function classifyTypeError(error) {
...
async function updatePipelineStatus(pipelineId, newStatus, reason) {
    // Get current state
    const { data: pipeline, error: fetchError } = await supabase
        .from('pipelines')
        .select('status')
...
// ═══════════════════════════════════════════════════════════════════
async function validateAndWriteFile(filePath, content, projectRoot) {
 ...
 */
function validateAndFixImportOrder(code, filePath) {
...
async function parseAndWriteFiles(codeBlock, localPath, pipelineId) {
...
async function runCoderStep(pipeline, repoPath) {
    // 1. CHECKPOINT CHECK: Skip if already completed
    ...
    const matrix = plannerStep?.output?.matrix || {
    ...
    try {
        ...
        if (intent.isHybrid || (intent.isPython && isNewProject)) {
            ...
            const designContext = {
                ...
                aesthetic: "premium", // 'modern' | 'minimal' | 'premium'
...
PRIORITY: Visual accuracy to the reference image > Generic design rules.
        `;
            ...
            let backendModel = "BACKEND"; // Default (DeepSeek/Qwen)
...
... code ...
          `;
                    ...
                    const result = await generateWithValidation(pipeline.id, 'coder', fullPrompt, targetFile, repoPath, 10 // Max attempts
                    );
                    if (!result.success) {
                ...
                if (isNewProject) {
                    ...
                    const result = await generateWithValidation(pipeline.id, 'coder', `${systemContext}\n\n${taskPrompt}`, 'PROJECT_ROOT', // Virtual file for full project generation
                    repoPath, 10 // Max attempts
                    );
                    if (!result.success) {
                        await logEvent(pipeline.id, 'VALIDATION_FAILED', 'coder', {
                            attempts: result.attempts,
                    ...
                    if (pipeline.type === 'update') {
                        ...
                        if (isDesignTask) {
                            ...
                            const result = await generateWithValidation(pipeline.id, 'coder', `${systemContext}\n\n${taskPrompt}`, 'UI_COMPONENTS', repoPath, 10);
                            if (!result.success) {
                                await logEvent(pipeline.id, 'VALIDATION_FAILED', 'coder', {
                                    attempts: result.attempts,
                                    issues: result.issues,
                            ...
                            const result = await generateWithValidation(pipeline.id, 'coder', `${systemContext}\n\n${taskPrompt}`, 'LOGIC_FILES', repoPath, 8 // Fewer attempts for logic tasks
                            );
                            if (!result.success) {
                                await logEvent(pipeline.id, 'VALIDATION_FAILED', 'coder', {
                                    attempts: result.attempts,
                        ...
                        const result = await generateWithValidation(pipeline.id, 'coder', `${systemContext}\n\n${taskPrompt}`, 'GENERAL_FILES', repoPath, 8);
                        if (!result.success) {
                            await logEvent(pipeline.id, 'VALIDATION_FAILED', 'coder', {
                                attempts: result.attempts,
                                issues: result.issues
                ...
                if (!validation.valid) {
                    ...
                    const fixedOutput = validation.fixes.map(f => `### FILE: ${f.file}\n\`\`\`tsx\n${f.content}\n\`\`\`\n### END_FILE`).join('\n\n');
                ...
                if (filesCreated === 0)
                    throw new Error("AI generated 0 valid files.");
            }
            // =============================================================================
            ...
            if (!layoutContent.includes('RefreshProvider')) {
                ...
                if (layoutContent.includes("import")) {
                    ...
                    if (imports && imports.length > 0) {
                        ...
                        layoutContent = layoutContent.replace(lastImport, `${lastImport}${importStatement}\n`);
                    }
                    else {
                ...
                if (layoutContent.includes('return')) {
                    ...
                    layoutContent = layoutContent.replace(/(return\s*\(?\s*)(<[^>]*>[\s\S]*?)(\{children\})([\s\S]*?)(<\/[^>]*>\s*\)?;?)/, (match, returnStart, beforeChildren, children, afterChildren, closing) => {
                    ...
                    if (!layoutContent.includes('<RefreshProvider>')) {
                        layoutContent = layoutContent.replace(/(\{children\})/g, '<RefreshProvider>{children}</RefreshProvider>');
                    }
                }
                else {
                    // Fallback: lägg till RefreshProvider runt hela body om det finns
                    if (layoutContent.includes('<body')) {
                        layoutContent = layoutContent.replace(/(<body[^>]*>)([\s\S]*?)(<\/body>)/, (match, openBody, bodyContent, closeBody) => {
...
async function runSqlStep(pipeline, repoPath) {
...
// ------------------------------------------------------------------
async function runAuditLoop(pipeline, repoPath) {
 ...
 */
function detectLazyPythonPass(content, filePath) {
    const lines = content.split('\n');
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i].trim();
...
function scanForLaziness(projectPath) {
    console.log("🕵️ Scanning for lazy code...");
    const issues = [];
    const scannedFiles = [];
    const consoleLogWarnings = [];
    const pythonFiles = [];
    const tsFiles = [];
    // Collect all files
    function scanDirectory(dir, baseDir = projectPath) {
        try {
 ...
 */
function standardizeExports(dir) {
    if (!fs.existsSync(dir))
        return;
    const files = [];
...
async function runIntelligentBatchFixer(repoPath, initialError, pipeline) {
    ...
    while (true) {
        ...
        if (!canRetry) {
            ...
            if (pipelineId) {
                try {
                    await updatePipelineStatus(pipelineId, 'failed_hard', currentError.substring(0, 1000) // Truncate for DB
                    );
                    await supabase
                        .from('pipeline_steps')
                        .update({
            ...
            const errorReportPath = path.join(repoPath, `error-reports/error-report-${new Date().toISOString()}.json`);
        ...
        const errorCategory = classified.classification;
 ...
 */
function verifyProductionReadiness(localPath, intent) {
...
`;
    ...
    if (fs.existsSync(path.join(localPath, '.eslintrc.json')) || fs.existsSync(path.join(localPath, 'eslint.config.mjs'))) {
        try {
            ...
            console.warn("   ⚠️ Lint warnings found (continuing)...");
        }
    }
    ...
    if (backendType === 'fastapi' || backendType === 'python') {
        ...
        try {
            ...
            pythonError.stdout = stdout;
            pythonError.stderr = stderr;
            pythonError.fullOutput = stdout + '\n' + stderr;
            pythonError.isPythonError = true;
            throw pythonError;
        }
    }
    else {
 ...
 */
function nukeNextJsCache(projectPath) {
    ...
    pathsToDelete.forEach(p => {
        if (fs.existsSync(p)) {
            try {
                ...
                console.warn(`   ⚠️ Failed to delete ${path.relative(projectPath, p) || path.basename(p)} (File locked?):`, e.message);
            }
        }
    });
 ...
 */
async function sanitizeAllCodeFiles(projectPath) {
...
function sanitizeMiddleware(projectPath) {
    ...
    if (!targetPath)
        return; // Ingen middleware = inget problem
    ...
    `;
        ...
        content = content.replace(/export (async )?function middleware\(request: NextRequest\) \{/, `export $1function middleware(request: NextRequest) {${bypassCode}`);
        // Ensure NextResponse is imported
        if (!content.includes('NextResponse')) {
            if (content.includes('from "next/server"') || content.includes("from 'next/server'")) {
                // Update existing import
                content = content.replace(/import\s+.*from\s+['"]next\/server['"]/, `import { NextRequest, NextResponse } from 'next/server'`);
            }
            else {
                // Add import at top
...
async function runTesterStep(pipeline, repoPath) {
    ...
    if (!global.testerAttempts) {
        global.testerAttempts = {};
    }
    global.testerAttempts[attemptKey] =
        (global.testerAttempts[attemptKey] || 0) + 1;
    if (global.testerAttempts[attemptKey] > MAX_TESTER_ATTEMPTS) {
    ...
    console.log(`[Tester] Attempt ${global.testerAttempts[attemptKey]}/${MAX_TESTER_ATTEMPTS}...`);
    // =============================================================================
    ...
    const hasRootFolders = ['app', 'components', 'lib', 'types'].some(folder => fs.existsSync(path.join(repoPath, folder)));
    if (hasRootFolders) {
...
`;
    ...
    if (fs.existsSync(sanitizePkgPath)) {
        try {
            ...
            if (pkg.devDependencies?.typescript === "5.0.0" || pkg.dependencies?.typescript === "5.0.0") {
                ...
                if (pkg.dependencies)
                    pkg.dependencies.typescript = "latest";
                modified = true;
            }
    ...
    const pipelineContext = createNewPipelineContext(pipeline.ticket_id || pipeline.id, pipeline.id, pipeline.initial_prompt || pipeline.prompt || "");
    ...
    while (attempt < maxRetries && !success) {
        ...
        try {
            ...
            try {
                ...
                console.warn("-> Warning: Could not create pnpm-workspace.yaml (non-critical):", e.message);
            }
            ...
            try {
                ...
                if (fs.existsSync(nextConfigPath)) {
                    ...
                    if (!nextConfigContent.includes('reactStrictMode')) {
                        ...
                        nextConfigContent = nextConfigContent.replace(/const nextConfig = \{/, `const nextConfig = {\n  reactStrictMode: true,`);
                        needsUpdate = true;
                    }
                    if (needsUpdate) {
...
`;
                ...
                console.warn("-> Warning: Could not update next.config.mjs (non-critical):", e.message);
            }
...
`;
                            ...
                            if (functionMatch) {
                                middlewareContent = middlewareContent.replace(functionMatch[0], functionMatch[0] + unblockCode);
                                needsUpdate = true;
                            }
                            else {
                                // Fallback: Lägg till efter första {
                                middlewareContent = middlewareContent.replace(/(export async function middleware\([^)]*\)\s*\{)/, `$1${unblockCode}`);
                                needsUpdate = true;
                            }
                        }
                        ...
                        if (!middlewareContent.includes("'/((?!api|_next/static|_next/image|favicon.ico|$).*)'")) {
                            ...
                            if (matcherMatch) {
                                ...
                                middlewareContent = middlewareContent.replace(/matcher:\s*\[[^\]]*\]/, `matcher: [
    '/((?!api|_next/static|_next/image|favicon.ico|.*\\.(?:svg|png|jpg|jpeg|gif|webp)$).*)',
                                ...
                                middlewareContent = middlewareContent.replace(/(export const config\s*=\s*\{)/, `$1\n  matcher: [\n    '/((?!api|_next/static|_next/image|favicon.ico|.*\\.(?:svg|png|jpg|jpeg|gif|webp)$).*)',\n  ],`);
                                needsUpdate = true;
                            }
                        }
                        if (needsUpdate) {
                ...
                console.warn("-> Warning: Could not update middleware.ts (non-critical):", e.message);
            }
            ...
            try {
                ...
                if (qualityError.isPythonError && fullQualityLog.includes('error:')) {
                    ...
                    try {
                        ...
                        const fixed = await autoFixPythonError(errorAnalysis, repoPath);
                        if (fixed) {
                ...
                if (fullQualityLog.includes('error TS') && shouldUseBatchSurgeon(fullQualityLog)) {
                    ...
                    try {
                        const batchResult = await runBatchSurgeon(fullQualityLog, repoPath, enrichedContext.availableComponents, parseAndWriteFiles);
...
`;
                ...
                if (typeof wasFixing !== 'undefined' && wasFixing && lastAppliedFix && lastErrorLog) {
                    ...
                    await memorizeSolution(lastErrorLog, lastAppliedFix).catch(err => console.warn("⚠️ Failed to memorize solution:", err?.message));
                }
...
### END_FILE
          `;
            ...
            const currentAttempts = currentStep?.attempts || 0;
            ...
            recordAutopsyError(errorAnalysis.errorCode, fullLog.slice(0, 200), fullLog.match(/([a-zA-Z0-9_\/\\.-]+\.(tsx|ts))/)?.[1] || 'unknown', `Attempt ${attempt}`);
            ...
            if (shouldUseBatchSurgeon(fullLog)) {
                ...
                try {
                    const batchResult = await runBatchSurgeon(fullLog, repoPath, enrichedContext.availableComponents, parseAndWriteFiles);
                    if (batchResult.totalFixed > 0) {
            ...
            if (loopCheck.isLooping && loopCheck.count >= 2) {
                ...
                const brokenContent = fs.existsSync(path.join(repoPath, brokenFilePath))
                    ...
                    : '';
                try {
                    const { diagnosis, fixPrompt } = await runErrorAutopsy(fullLog, brokenFilePath, brokenContent, repoPath);
          ...
          `;
            ...
            if (hasAsyncError && brokenFileContent) {
                ...
                if (brokenFileContent.match(/const\s*\{\s*\w+\s*\}\s*=\s*params\s*[;}]/)) {
                    ...
                    newContent = newContent.replace(/const\s*\{([^}]+)\}\s*=\s*params\s*([;}]?)/g, "const { $1 } = await params$2");
                    wasModified = true;
                }
                // Fix params.property access
                if (brokenFileContent.match(/params\.\w+/)) {
                    // Match: params.slug or params.id
                    newContent = newContent.replace(/const\s+(\w+)\s*=\s*params\.(\w+)/g, "const $1 = (await params).$2");
                    wasModified = true;
                }
                // Fix searchParams
                if (brokenFileContent.includes("searchParams")) {
                    newContent = newContent.replace(/const\s*\{\s*([^}]+)\s*\}\s*=\s*searchParams\s*([;}]?)/g, "const { $1 } = await searchParams$2");
                    newContent = newContent.replace(/searchParams\.get\(/g, "(await searchParams).get(");
                    wasModified = true;
                }
                ...
                if (wasModified && newContent !== brokenFileContent) {
                    ...
                    if (newContent.match(/export\s+function\s+\w+\s*\(/)) {
                        newContent = newContent.replace(/export\s+function\s+(\w+)\s*\(/g, "export async function $1(");
                    }
                    // Safe Write: Ensure directory exists
      ...
      `;
            ...
            let strategy = "PATCH";
            let steps = [];
            let targetFiles = [brokenFile || 'unknown'];
            try {
                ...
                if (strategyJson.toLowerCase().includes('rewrite'))
                    strategy = "REWRITE";
                else if (strategyJson.toLowerCase().includes('install'))
                    strategy = "INSTALL";
                else if (strategyJson.toLowerCase().includes('export') || strategyJson.toLowerCase().includes('import'))
                    strategy = "ALIGN_EXPORTS";
            }
            console.log(`🎯 Strategy: ${strategy}`);
            if (steps.length > 0)
                console.log(`📝 Steps: ${steps.slice(0, 3).join(', ')}...`);
            // ------------------------------------------------------------
            ...
            const relevantIntel = knowledgeGraph.files.filter(node => fullLog.includes(node.path) || // File mentioned in error
...
`;
      ...
      `;
            ...
            if (fixedCount === 0)
                console.log("[Watchdog] ⚠️ No code changes applied.");
        ...
        if (hasFrontend) {
            ...
            try {
                ...
                try {
                    ...
                    const finalResult = await refinementLoop(repoPath, devServerUrl, 3, // Max iterations
                    async (filePath, code) => {
                    ...
                    if (serverProc) {
                        try {
                            ...
                            console.warn(chalk.yellow(`   ⚠️ Could not stop dev server: ${e.message}`));
                        }
                    }
                }
            }
 ...
 */
async function generateRepoName(userRequest) {
    const prompt = `
...
const RECOVERY_STRATEGIES = {
    ...
    TYPESCRIPT_ERROR: {
        ...
        detect: (error) => error.includes('Type error') || error.includes('TS'),
        fix: async (projectRoot, pipeline) => {
    ...
    MODULE_RESOLUTION: {
        ...
        detect: (error) => error.includes('Module not found') || error.includes('Cannot resolve'),
        fix: async () => {
    ...
    HOMEPAGE_404: {
        ...
        detect: (error) => error.includes('Homepage renders 404') || error.includes('This page could not be found'),
        fix: async (projectRoot, pipeline) => {
 ...
 */
async function attemptBuildRecovery(error, projectRoot, pipeline) {
 ...
 */
function addForceDynamic(content) {
    const lines = content.split('\n');
    const useClientIndex = lines.findIndex(l => l.trim() === "'use client';" || l.trim() === '"use client";');
    if (useClientIndex >= 0) {
        lines.splice(useClientIndex + 1, 0, '', '// Force dynamic rendering', "export const dynamic = 'force-dynamic';", "export const dynamicParams = true;", '');
    }
    else {
        // No 'use client', add at the top
        lines.unshift('', '// Force dynamic rendering', "export const dynamic = 'force-dynamic';", "export const dynamicParams = true;", '');
    }
    return lines.join('\n');
}
/**
 ...
 */
async function convertToClientComponent(content) {
 ...
 */
async function attemptHomepage404Fix(projectRoot, pipeline) {
...
function checkImportOrderIssue(content) {
    const lines = content.split('\n');
    let firstImportIndex = -1;
    let firstExportIndex = -1;
    for (let i = 0; i < lines.length; i++) {
...
async function clearNextCache(projectRoot) {
    const cachePaths = [
 ...
 */
async function callAIToFixPage(pagePath, projectRoot, pipeline) {
    try {
 ...
 */
function extractMissingDependencies(error) {
    const matches = [];
 ...
 */
async function fixBuildErrorWithAI(error, projectRoot, pipeline) {
    try {
        const prompt = `
...
function generateDocs(repoPath, pipeline) {
 ...
 */
async function ensureEnvExample(repoPath) {
    const envPath = path.join(repoPath, '.env.local');
    const envExamplePath = path.join(repoPath, '.env.example');
 ...
 */
async function verifySitemap(repoPath) {
 ...
 */
async function validateNextConfig(projectRoot) {
    ...
    try {
        ...
        if (!configContent.includes('appDir') && !configContent.includes('experimental')) {
            ...
            if (configContent.includes('experimental:')) {
                updatedConfig = configContent.replace(/experimental:\s*\{/, 'experimental: {\n    appDir: true,');
                needsUpdate = true;
            }
            else {
                // Add experimental block after reactStrictMode or at the start
                if (configContent.includes('reactStrictMode')) {
                    updatedConfig = configContent.replace(/reactStrictMode:\s*true,?/, 'reactStrictMode: true,\n  experimental: {\n    appDir: true,\n  },');
                }
                else {
                    updatedConfig = configContent.replace(/const nextConfig = \{/, `const nextConfig = {\n  experimental: {\n    appDir: true,\n  },`);
                }
                needsUpdate = true;
            }
        }
        ...
        if (!configContent.includes('reactStrictMode')) {
            ...
            updatedConfig = updatedConfig.replace(/const nextConfig = \{/, 'const nextConfig = {\n  reactStrictMode: true,');
            needsUpdate = true;
        }
...
async function analyzeBuildOutput(buildOutput) {
 ...
 */
async function runFinalBuildVerification(workspacePath, pipelineId, pipeline) {
 ...
 */
async function findBrokenLinks(workspacePath) {
    const brokenLinks = [];
    ...
    for (const file of files) {
        ...
        while ((match = linkRegex.exec(content)) !== null) {
            ...
            const pagePath = path.join(workspacePath, 'src/app', href === '/' ? 'page.tsx' : `${href}/page.tsx`);
            if (!fs.existsSync(pagePath)) {
 ...
 */
async function generateStubPages(workspacePath, brokenLinks) {
 ...
 */
async function validateAndFixDependencies(workspacePath) {
 ...
 */
async function generateProductionReadme(workspacePath, projectName) {
    const pkgJson = JSON.parse(fs.readFileSync(path.join(workspacePath, 'package.json'), 'utf-8'));
...
`;
    fs.writeFileSync(path.join(workspacePath, 'README.md'), readme, 'utf-8');
}
async function runPublisherStep(pipeline, repoPath) {
...
// STEG 7: CLEANUP
// ------------------------------------------------------------------
async function runCleanupStep(pipeline, repoPath) {
    ...
    try {
        ...
        console.warn("[Cleanup] ⚠️ Could not kill processes (non-critical):", e.message);
    }
...
// MAIN LOOP
// ------------------------------------------------------------------
export async function runPipelineLoop(sandboxPath) {
            ...
            switch (pipeline.current_phase) {
                ...
                case 'research':
                    ...
                    try {
                        ...
                        const k2Synthesis = await runK2SynthesisStep(pipeline.id, pipeline.initial_prompt || pipeline.prompt || '', [perplexityReport1, perplexityReport2]);
