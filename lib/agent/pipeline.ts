...
export async function nextPipelineStage(
  taskId: number,
  currentStage: "planner" | "coder" | "reviewer" | "fixer" | "done" | "start_pipeline"
) {
  ...
  if (currentStage === "start_pipeline") {
    ...
    const plan = await generateContent(
      `Create a technical implementation plan for: "${userPrompt}". 
      Focus on component structure, props, and state.`,
  ...
  if (currentStage === "planner") {
    ...
    const code = await generateContent(
      `Write a single-file React Functional Component using Tailwind CSS based on this plan:
      ...
      3. Return ONLY the code. Start with 'import React'.`,
  ...
  if (currentStage === "coder") {
    ...
    const review = await generateContent(
      `Review this code. Focus on Logic, Security, and Best Practices. 
  ...
  if (currentStage === "reviewer") {
    ...
    const fixedCode = await generateContent(
      `Refactor this code based on the review.

      Original: ${originalCode}

      Review: ${feedback}

      Keep it in TypeScript. Return ONLY the code.`,
  ...
  if (currentStage === "fixer") {
    ...
    const polishedCode = await generateContent(
      `Fix ALL TypeScript errors in this code.
      ...
      5. Return ONLY the fully working .tsx code.`,
      "You are a TypeScript Compiler Humanizer."
    );
