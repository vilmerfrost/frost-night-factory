...
export async function executeAgentAction(action) {
    ...
    let logDetails = "";
    if ("path" in action)
        logDetails = action.path;
    else if ("dir" in action)
        logDetails = action.dir;
    else if ("stage" in action)
        logDetails = action.stage;
    else if ("task_id" in action)
        logDetails = `task_${action.task_id}`;
    else if ("message" in action)
        logDetails = action.message;
    else if ("content" in action)
        logDetails = `content_${action.content.slice(0, 20)}...`;
