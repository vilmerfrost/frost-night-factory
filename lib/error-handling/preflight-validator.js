 ...
 */
async function runTypeScriptCheck(workspace) {
    const errors = [];
    try {
 ...
 */
async function validateImportGraph(workspace) {
    const errors = [];
 ...
 */
async function validateTypeRegistry(workspace) {
    const missing = [];
    const srcDir = path.join(workspace, 'src');
    try {
        const files = await findTypeScriptFiles(srcDir);
 ...
 */
export async function runEnhancedPreFlight(workspace) {
    const errors = [];
        ...
        tsResult.errors.forEach(err => {
            if (err.code === 'TS2304') {
                ...
                if (match) {
                    ...
                    if (suggestedImport) {
                        errors.push(`Missing import in ${err.file}:${err.line} - ${suggestedImport}`);
                    }
                    else {
                        errors.push(`Undefined type '${typeName}' in ${err.file}:${err.line} (not in type registry)`);
                    }
                }
                else {
 ...
 */
export async function autoFixPreFlightIssues(workspace) {
    const errors = [];
    let fixed = 0;
    const srcDir = path.join(workspace, 'src');
    try {
