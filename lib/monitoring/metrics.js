...
// HELPER FUNCTIONS
// =============================================================================
export function incQuarantineReceived() {
    quarantineReceived?.inc();
}
export function incQuarantineRejected(reason) {
    quarantineRejected?.inc({ reason: reason.substring(0, 50) });
}
export function incQuarantineValidated() {
    quarantineValidated?.inc();
}
export function recordValidationDuration(seconds) {
    quarantineValidationDuration?.observe(seconds);
}
export function incPipelinePhaseTransition(from, to, status) {
...
export function incErrorLoopDetection(errorType) {
    errorLoopDetections?.inc({ error_type: errorType });
}
export function incPipelineError(errorType, phase) {
    pipelineErrors?.inc({ error_type: errorType, phase });
}
export function recordPipelineDuration(seconds, phase) {
    pipelineDuration?.observe({ phase }, seconds);
}
export function incMetamorphicValidation(result) {
    metamorphicValidations?.inc({ result });
}
export function recordMetamorphicConsistency(score) {
