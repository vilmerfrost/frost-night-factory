...
export class CodebaseOracle {
     ...
     */
    async askThinkingOracle(errorLog, fileContent) {
...
`;
     ...
     */
    findFileByExport(exportName) {
        const graph = this.getKnowledgeGraph();
        return graph.files.find(f => f.exports.includes(exportName) ||
            (exportName === 'default' && f.hasDefaultExport));
    }
    /**
     * Find all files that import a specific module
     */
    findFilesThatImport(modulePath) {
        const graph = this.getKnowledgeGraph();
        return graph.files.filter(f => f.imports.some(imp => imp.includes(modulePath)));
    }
    /**
     * Get relevant files for an error (for token-efficient prompts)
     */
    getRelevantFilesForError(errorLog) {
        const graph = this.getKnowledgeGraph();
        return graph.files.filter(node => errorLog.includes(node.path) || // File mentioned in error
     ...
     */
    classifyFile(relativePath, content) {
...
export function getOracle(projectPath) {
