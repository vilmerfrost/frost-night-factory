 ...
 */
function extractPackageImports(filePath: string): Set<string> {
 ...
 */
function scanProjectForPackages(projectPath: string): Set<string> {
  const packages = new Set<string>();
 ...
 */
function getInstalledPackages(projectPath: string): { dependencies: Set<string>; devDependencies: Set<string> } {
...
export function runDependencyDetective(projectPath: string): DependencyScanResult {
  ...
  try {
    if (regularDeps.length > 0) {
      ...
      execSync(
        `npm install ${regularDeps.join(' ')} --legacy-peer-deps`,
    ...
    if (devDeps.length > 0) {
      ...
      execSync(
        `npm install --save-dev ${devDeps.join(' ')} --legacy-peer-deps`,
        { cwd: projectPath, stdio: 'pipe', timeout: 120000 }
      );
