 ...
 */
async function generateDocsInBackground(prompt, projectPath) {
    try {
...
export async function runDocumentationStep(projectPath, techStack) {
    console.log("📝 Queuing Documentation Generation (Batch Mode)...");
    // 1. Läs in filstruktur och nyckelfiler för kontext
    const fileList = [];
    function collectFiles(dir, baseDir = projectPath) {
        try {
    ...
    try {
        ...
        console.warn("[Documentation] Could not read some critical files:", e.message);
    }
    const prompt = `
  ...
  `;
    ...
    generateDocsInBackground(prompt, projectPath).catch(err => console.error("❌ Background Docs Failed:", err));
