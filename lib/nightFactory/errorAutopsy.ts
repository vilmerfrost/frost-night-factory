 ...
 */
export function detectLoop(currentError: string, currentFile: string): {
  isLooping: boolean;
  count: number;
  signature: string;
} {
  ...
  const count = recent.filter(e => 
    e.file === currentFile && 
    e.message.slice(0, 50) === currentError.slice(0, 50)
  ).length;
 ...
 */
export function getPreviousFixes(errorSignature: string): string[] {
  return errorHistory
...
`;
  ...
  try {
    ...
    console.error(`   ❌ Autopsy failed: ${(e as Error).message}`);
  }
 ...
 */
export function generateNuclearFixPrompt(
 ...
 */
export async function runErrorAutopsy(
  errorLog: string,
  file: string,
  fileContent: string,
  projectPath: string
  ...
  // Perform autopsy
  const diagnosis = await performAutopsy(
    errorLog,
    file,
    fileContent,
    previousFixes,
  ...
  // Generate fix prompt
  const fixPrompt = generateNuclearFixPrompt(
    diagnosis,
    file,
    fileContent,
    '' // Project context will be added by caller
  );
 ...
 */
export function shouldPerformAutopsy(currentError: string, currentFile: string): boolean {
