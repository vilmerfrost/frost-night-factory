 ...
 */
export function saveErrorPatterns(patterns) {
    try {
 ...
 */
function mergePatterns(known, loaded) {
    ...
    for (const p of loaded) {
        if (merged.has(p.id)) {
            ...
            existing.lastSeen = new Date(Math.max(new Date(existing.lastSeen).getTime(), new Date(p.lastSeen).getTime()));
        }
        else {
            merged.set(p.id, p);
        }
 ...
 */
export function recordErrorOccurrence(error, category, fix) {
 ...
 */
export function generatePreventionPrompt(minOccurrences = 2) {
    const patterns = loadErrorPatterns();
 ...
 */
export function getAutoFixSuggestion(error) {
    const patterns = loadErrorPatterns();
