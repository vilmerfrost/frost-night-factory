 ...
 */
export async function verifyDataFlow(context: PipelineContext, stage: string): Promise<void> {
...
`;
    ...
    try {
      const verdict = await callAI(
        "FLOW_WATCHER", 
        prompt, 
        "You are a Data Integrity Guardian. Be strict about MUST requirements, lenient about OPTIONAL.",
        contextDump
      );
 ...
 */
export function quickFlowCheck(context: PipelineContext, stage: string): boolean {
 ...
 */
export function logContextState(context: PipelineContext): void {
