 ...
 */
export async function checkNpmVersion(pkg, version) {
    try {
 ...
 */
export function getGoldenVersion(pkg, requestedVersion) {
    // Check if we have a golden version
    if (pkg in GOLDEN_VERSIONS) {
        return GOLDEN_VERSIONS[pkg];
    }
 ...
 */
export function validateAndFixDependencies(dependencies) {
    const fixed = {};
    const corrections = [];
 ...
 */
export function generateGoldenPackageJson(projectName = 'my-app') {
    return {
        name: projectName,
        version: "0.1.0",
        private: true,
