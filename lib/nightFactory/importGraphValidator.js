 ...
 */
function buildExportMap(projectPath) {
 ...
 */
function validateImports(projectPath, exportMap) {
    const issues = [];
    function scanDirectory(dir) {
        if (!fs.existsSync(dir))
            return;
 ...
 */
function autoFixIssues(projectPath, issues) {
    let fixedCount = 0;
            ...
            if (issue.issue.includes('no default export') && issue.suggestion.includes('Use named import')) {
                ...
                if (namedMatch) {
                    ...
                    if (line.includes(oldImport)) {
                        ...
                        content = content.replace(new RegExp(`\\b${issue.importedNames[0]}\\b`, 'g'), newName);
                        fixedCount++;
                    }
                }
            }
            ...
            if (issue.issue.includes('not found in module exports') && issue.suggestion.includes('Use default import')) {
                ...
                if (defaultMatch) {
                    ...
                    if (line.includes(oldImport)) {
                        contentLines[lineIndex] = line.replace(oldImport, newImport);
                        // Also replace usage
                        content = content.replace(new RegExp(`\\b${issue.importedNames[0]}\\b`, 'g'), defaultName);
                        fixedCount++;
                    }
                }
            }
 ...
 */
export function runImportGraphValidator(projectPath) {
