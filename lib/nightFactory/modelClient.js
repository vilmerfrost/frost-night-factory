 ...
 */
export async function generateClaudeHaikuJSON(rawText, jsonSchema, phase, maxRetries = 2) {
    ...
    if (!anthropic) {
        ...
        return fallbackToGeminiJSON(rawText, jsonSchema, phase, auditTrail);
    }
...
2. NO markdown, NO code fences, NO explanations, NO text before/after, NO "I understand", NO "Here is", NO "Let me", NO end notes, NO acknowledgments
3. NO comments, NO explanations, NO conversational text
4. Start with { and end with } - ABSOLUTELY NOTHING before or after
5. Follow the schema EXACTLY
6. If data is missing, use null or empty arrays
7. If the input contains JSON code, ESCAPE it properly in string values - do NOT include it as raw JSON
8. NEVER wrap output in \`\`\`json\`\`\` code blocks
9. NEVER include double commas (",",) - this creates invalid JSON
10. CRITICAL: When raw text contains JSON code (like "phase": "research",), escape it as a STRING value, not as raw JSON

TARGET SCHEMA:
${jsonSchema}

REMEMBER: Your response must be PURE JSON ONLY. Any text before or after the JSON will cause parsing to fail.`;
...
3. NO explanations, NO "I understand", NO "Here is the JSON", NO acknowledgments, NO end notes
4. NO comments, NO conversational text
5. Start directly with { and end with } - ABSOLUTELY NOTHING before or after
...
8. CRITICAL: If the raw input contains JSON code (like "phase": "research",), you MUST escape it as a STRING value in the JSON output
9. NEVER include double commas (",",) - this creates invalid JSON that cannot be parsed
10. Example: If raw text contains {"phase": "research",}, output it as "full_raw_output": "{\\"phase\\": \\"research\\",}" (escaped)
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            const msg = await anthropic.messages.create({
                model: "claude-haiku-4-5-20250514", // Claude 4.5 Haiku (2025)
                max_tokens: 4096,
            ...
            // ═══════════════════════════════════════════════════════════════════
            // 🔧 STEP 1: Remove markdown code fences if Claude added them
            // ═══════════════════════════════════════════════════════════════════
            // Remove markdown code blocks if present (shouldn't happen with zero temp)
            ...
            // ═══════════════════════════════════════════════════════════════════
            // 🔧 STEP 2: Basic sanitization (minimal - no aggressive repair)
            // ═══════════════════════════════════════════════════════════════════
            ...
            // ═══════════════════════════════════════════════════════════════════
            // 🔧 STEP 3: Basic JSON repair (only fix obvious issues)
            // ═══════════════════════════════════════════════════════════════════
            
            // Fix trailing commas (common and safe to fix)
            ...
            try {
                ...
                // Basic repair: Only fix obvious issues
                let repaired = sanitized;
                // Fix trailing commas (safe to fix)
            ...
            if (parsed.error) {
                throw new Error(`Model returned error: ${parsed.error}`);
            }
 ...
 */
async function fallbackToGeminiJSON(rawText, jsonSchema, phase, auditTrail) {
    if (!geminiModel) {
        return {
            success: false,
...
OUTPUT FORMAT: Start immediately with { and end with }. No other text.`;
        ...
        // JSON REPAIR: Fix double comma issues
        let sanitized = jsonStr
            .replace(/[\r\n]+/g, ' ')
            .replace(/  +/g, ' ')
            .trim();
...
export async function summarizeText(content) {
    if (!geminiModel)
        return "⚠️ MOCK SUMMARY (No Google Key)";
    try {
...
export async function generateContent(prompt, systemPrompt = "") {
    if (!geminiModel)
        return `⚠️ MOCK CONTENT (No Google Key) for: ${prompt}`;
...
// ============================================================
export async function performDeepResearch(topic) {
    const apiKey = process.env.PERPLEXITY_API_KEY;
    if (!apiKey) {
        console.log("⚠️ No Perplexity Key found. Falling back to Gemini.");
        return generateContent(`Perform deep technical research on: ${topic}`, "You are a Senior Technical Researcher");
    }
    ...
    try {
        const response = await perplexity.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: "You are a Senior Technical Researcher. Search for the latest documentation. Be extremely technical and specific about implementation details, libraries, and versions.",
                },
 ...
 */
export async function generateClaudeCoder(prompt, systemPrompt) {
 ...
 */
export async function generateDeepSeekPlanner(prompt) {
    ...
    try {
        const completion = await callWithRetry(() => deepSeek.chat.completions.create({
 ...
 */
export async function generateDeepSeekCoder(prompt) {
    // Fallback till Gemini om ingen DeepSeek-nyckel
    if (!deepSeek) {
 ...
 */
export async function generateKimiPlanner(prompt) {
 ...
 */
export async function runKimiQA(codeSnippets) {
 ...
 */
export async function generateLocalCoder(prompt, systemPrompt) {
    try {
 ...
 */
export async function generateLocalReview(codeSnippet, instructions) {
 ...
 */
export async function generateBuildFix(errorLog, fileContext) {
 ...
 */
export async function generateLocalFix(errorLog, brokenFileContent) {
    // 1. FÖRSÖK MED LOCALHOST (Gratis)
    try {
 ...
 */
export async function generateGroqFix(errorLog, brokenFileContent) {
    if (!groq) {
...
export async function callAI(role, prompt, context, imageBase64, smartLevel = 'FAST') {
    ...
    switch (role) {
        ...
        case "PLANNER":
            ...
            if (deepSeek) {
                try {
                    ...
                    const planner = await withTimeout(deepSeek.chat.completions.create({
        ...
        case "FRONTEND":
            ...
            if (!claudeSuccess) {
                ...
                try {
                    const deepSeekBackup = await callWithRetry(() => deepSeek.chat.completions.create({
        ...
        case "BACKEND":
            ...
            if (deepSeek) {
                try {
                    ...
                    const startTime = Date.now();
                    const backendResult = await callWithRetry(() => withTimeout(deepSeek.chat.completions.create({
        ...
        case "DEBUGGER":
            ...
            if (deepSeek) {
                try {
                    const startTime = Date.now();
                    const reviewResult = await callWithRetry(() => withTimeout(deepSeek.chat.completions.create({
                        ...
                        temperature: 0.3,
                        max_tokens: 8000
                    }), reviewTimeout, `DeepSeek V3.2 ${role}`), 3, 2000);
                    const elapsed = Date.now() - startTime;
