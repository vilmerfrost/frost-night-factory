...
export async function ensurePipelineState(taskId) {
...
export async function canRunAgent(taskId, role) {
    ...
    if (role === "coder" && !state.planner_completed)
        return false;
...
export async function markAgentCompleted(taskId, role, runId, agentRunId, status, errorMessage) {
...
export async function createAgentRunLog(taskId, runId, role) {
    const { data, error } = await supabase
        .from("night_factory_agent_runs")
        .insert({
        task_id: taskId,
