...
import { getGoldenTemplate } from './goldenTemplates';
/**
 * Run all pre-commit validation checks
 */
export async function runPreCommitValidation(projectDir, autoFix = true) {
 ...
 */
async function validatePackageJson(projectDir, autoFix) {
    ...
    try {
        ...
        for (const [name, version] of Object.entries(allDeps)) {
            const goldenVersion = GOLDEN_VERSIONS[name];
 ...
 */
async function validateTsConfig(projectDir, autoFix) {
 ...
 */
async function validateExports(projectDir, autoFix) {
    const errors = [];
 ...
 */
async function scanForLazyCode(projectDir) {
    const errors = [];
    const lazyPatterns = [
 ...
 */
async function validateImports(projectDir) {
    const errors = [];
 ...
 */
async function validateRequiredFiles(projectDir, autoFix) {
    const errors = [];
    let fixApplied = false;
    const requiredFiles = [
        'package.json',
 ...
 */
function checkComponentCasing(fileContent) {
    const errors = [];
 ...
 */
async function validateComponentCasing(projectDir, autoFix) {
    const errors = [];
    let fixApplied = false;
    const fixes = [];
    // Common HTML tags that are lowercase by design
        ...
        if (fileErrors.length > 0) {
            ...
            if (autoFix) {
                ...
                for (const [lowercase, capitalized] of tagReplacements.entries()) {
                    // Opening tags
                    content = content.replace(new RegExp(`<${lowercase}(?=\\s|>|/)`, 'g'), `<${capitalized}`);
                    // Closing tags
                    content = content.replace(new RegExp(`</${lowercase}>`, 'g'), `</${capitalized}>`);
