 ...
 */
export function extractExportsFromCode(content) {
 ...
 */
export function buildExportMap(files) {
 ...
 */
export function validateImports(files, exportMap) {
    const issues = [];
    for (const file of files) {
        if (!file.path.match(/\.(tsx?|jsx?)$/))
            continue;
 ...
 */
export function autoFixImportExportMismatches(files, issues) {
    ...
    for (const issue of issues) {
        ...
        if (issue.issue?.includes('not found in module exports') && issue.suggestion.includes('Use default import')) {
            ...
            if (defaultName) {
                ...
                const namedImportRegex = new RegExp(`import\\s+\\{[^}]*\\b${issue.importedNames[0]}\\b[^}]*\\}\\s+from\\s+['"]${issue.importPath.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}['"]`);
 ...
 */
export function runSelfAwareValidation(files) {
 ...
 */
export function generateExportReport(files) {
    const exportMap = buildExportMap(files);
    const lines = ['=== EXPORT MAP ==='];
