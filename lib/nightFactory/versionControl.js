 ...
 */
export function createSnapshot(projectPath, description = 'Auto-snapshot') {
 ...
 */
export function restoreFromSnapshot(projectPath, snapshotId) {
    const snapshotsDir = path.join(projectPath, '.nightfactory-snapshots');
    const snapshotPath = path.join(snapshotsDir, snapshotId);
    if (!fs.existsSync(snapshotPath)) {
 ...
 */
export function cleanupSnapshots(projectPath, keepCount = 5) {
    const snapshots = listSnapshots(projectPath);
 ...
 */
export function snapshotBeforeTester(projectPath) {
    return createSnapshot(projectPath, 'Before tester step');
}
/**
 * Create snapshot before fixer operations
 */
export function snapshotBeforeFixer(projectPath) {
