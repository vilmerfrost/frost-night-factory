 ...
 */
async function fallbackTextAudit(projectDescription: string): Promise<VisionAuditResult> {
...
export async function refinementLoop(
  ...
  onCodeUpdate?: (filePath: string, code: string) => Promise<void>
  ...
  while (iterations < maxIterations && currentScore < 8) {
    ...
    if (auditResult.suggestions.length > 0 && onCodeUpdate) {
      ...
      await applyFixes(projectPath, auditResult.suggestions, async (filePath: string, code: string) => {
 ...
 */
export async function startDevServerWithVerification(
  projectPath: string,
  port: number = 3002
