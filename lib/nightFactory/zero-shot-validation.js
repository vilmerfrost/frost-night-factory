...
// =============================================================================
async function refinePromptLoop(originalPrompt) {
    ...
    while (attempts < maxRetries) {
        ...
        if (validation.riskScore > 80) {
            throw new Error(`🚨 SECURITY BLOCK: Request rejected due to high risk (${validation.riskScore}/100). Reason: ${validation.refusalReason}`);
        }
    ...
    if (!finalValidation.isValid || finalValidation.riskScore >= 30) {
        throw new Error(`❌ Failed to sanitize prompt after ${maxRetries} attempts. Last reason: ${finalValidation.refusalReason}`);
    }
    return currentPrompt;
}
export async function runZeroShotPipeline(userRequest, context) {
