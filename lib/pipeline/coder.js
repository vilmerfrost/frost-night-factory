 ...
 * Receives ResearchPhaseJSON + PlannerPhaseJSON, outputs CoderPhaseJSON
 */
export async function runCoderPhaseJSON(researchContext, planContext, repoPath, options = {}) {
    const { generateFrontend = true, generateBackend = true, useClaude = true } = options;
...
NO markdown, NO explanations. ONLY valid JSON.`;
        try {
            const backendResponse = await callAI("BACKEND", backendPrompt, "You are a Senior Backend Engineer specializing in TypeScript and APIs");
            rawCoderOutput += `\n=== BACKEND ===\n${backendResponse}\n`;
            // Parse backend files
...
`;
    ...
    result.data.code_generated.frontend.total_lines = frontendFiles.reduce((sum, f) => sum + f.content.split("\n").length, 0);
    result.data.code_generated.backend.files_count = backendFiles.length;
    result.data.code_generated.backend.total_lines = backendFiles.reduce((sum, f) => sum + f.content.split("\n").length, 0);
 ...
 */
export function legacyToCoderPhaseJSON(legacy, researchTimestamp, planTimestamp) {
    const frontendFiles = legacy.files.filter(f => f.path.startsWith("app/") && !f.path.includes("/api/") ||
        f.path.startsWith("components/"));
    const backendFiles = legacy.files.filter(f => f.path.includes("/api/") ||
