...
import { runResearchPhaseJSON, runPlannerPhaseJSON, runCoderPhaseJSON, runSqlPhaseJSON, runTesterPhaseJSON, buildPipelineContext, generateTraceabilityReport } from "./phases";
// ============================================================
// EXAMPLE 1: Full Pipeline (Recommended)
// ============================================================
async function runFullPipelineExample() {
    console.log("🚀 Running full pipeline...\n");
    const result = await runFullPipeline("Build an invoice management app with PDF upload and data extraction", "./workspace/invoice-app", {
        // Research options
        usePerplexity: true,
        useKimi: false,
        ticketType: "feature",
...
// ============================================================
async function runStepByStepExample() {
    ...
    const researchJSON = researchResult.data;
    ...
    const plannerJSON = plannerResult.data;
    ...
    const coderJSON = coderResult.data;
    ...
    const fullContext = buildPipelineContext({
...
// ============================================================
function traceTestFailure(context) {
    if (!context.tester) {
    ...
    for (const test of failedTests) {
        ...
        if (test.depends_on_feature && context.planner) {
            const feature = context.planner.feature_breakdown.phase_1_mvp.find(f => f.feature_id === test.depends_on_feature);
            if (feature) {
        ...
        // Show code files
        if (context.coder) {
            const relatedFiles = context.coder.features_implemented.find(f => f.feature_id === test.depends_on_feature);
            if (relatedFiles) {
...
// ============================================================
async function accessAuditTrail() {
    const result = await runFullPipeline("Build a simple todo app", "./workspace/todo-app", { stopOnError: false });
...
export { runFullPipelineExample, runStepByStepExample, traceTestFailure, accessAuditTrail };
