 ...
 */
export async function convertResearchToJSON(rawText, userPrompt) {
    // Pre-process: Add timestamp and user context
    const enhancedInput = `USER QUERY: ${userPrompt}\n\nRESEARCH OUTPUT:\n${rawText}`;
    const result = await generateClaudeHaikuJSON(enhancedInput, RESEARCH_SCHEMA, "research");
 ...
 */
export async function convertPlannerToJSON(rawText, researchContext) {
    // Pre-process: Include research reference
...
PLANNER OUTPUT:
${rawText}`;
    const result = await generateClaudeHaikuJSON(enhancedInput, PLANNER_SCHEMA, "planner");
    if (result.success && result.data) {
 ...
 */
export async function convertCoderToJSON(rawText, researchContext, planContext) {
    // Pre-process: Include both contexts
    const enhancedInput = `RESEARCH CONTEXT (from ${researchContext.timestamp}):
...
CODER OUTPUT:
${rawText}`;
    const result = await generateClaudeHaikuJSON(enhancedInput, CODER_SCHEMA, "coder");
    if (result.success && result.data) {
        result.data.timestamp = result.data.timestamp || new Date().toISOString();
 ...
 */
export async function convertSqlEditorToJSON(rawText, researchContext, planContext, coderContext) {
    // Pre-process: Full context
    const enhancedInput = `RESEARCH CONTEXT (from ${researchContext.timestamp}):
...
SQL EDITOR OUTPUT:
${rawText}`;
    const result = await generateClaudeHaikuJSON(enhancedInput, SQL_EDITOR_SCHEMA, "sql_editor");
    if (result.success && result.data) {
        result.data.timestamp = result.data.timestamp || new Date().toISOString();
 ...
 */
export async function convertTesterToJSON(rawText, context) {
...
TESTER OUTPUT:
${rawText}`;
    const result = await generateClaudeHaikuJSON(enhancedInput, TESTER_SCHEMA, "tester");
    if (result.success && result.data) {
        result.data.timestamp = result.data.timestamp || new Date().toISOString();
 ...
 */
export function validatePipelineJSON(json, expectedPhase) {
    const errors = [];
    if (!json.phase) {
 ...
 */
export function getPipelineContextSummary(context) {
    const parts = [];
    if (context.research) {
