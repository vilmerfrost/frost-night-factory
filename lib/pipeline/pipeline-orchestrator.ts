...
import { runResearchPhaseJSON, legacyToResearchPhaseJSON } from "./research";
import { runPlannerPhaseJSON, legacyToPlannerPhaseJSON } from "./planner";
import { runCoderPhaseJSON, legacyToCoderPhaseJSON } from "./coder";
import { runSqlPhaseJSON, legacyToSqlEditorPhaseJSON } from "./sql";
...
export interface PipelineOptions {
  ...
  // Pipeline control
  stopOnError?: boolean;
  skipPhases?: Array<"research" | "planner" | "coder" | "sql" | "tester">;
}
...
export async function runFullPipeline(
  ...
  let researchJSON: ResearchPhaseJSON | undefined;
  ...
  let plannerJSON: PlannerPhaseJSON | undefined;
  ...
  let coderJSON: CoderPhaseJSON | undefined;
  ...
  let sqlJSON: SqlEditorPhaseJSON | undefined;
  ...
  let testerJSON: TesterPhaseJSON | undefined;
...
function createMinimalResearchJSON(userPrompt: string): ResearchPhaseJSON {
  return {
    phase: "research",
...
function createMinimalPlannerJSON(userPrompt: string, research: ResearchPhaseJSON): PlannerPhaseJSON {
  return {
    phase: "planner",
    timestamp: new Date().toISOString(),
    input_references: {
...
function createMinimalCoderJSON(research: ResearchPhaseJSON, planner: PlannerPhaseJSON): CoderPhaseJSON {
  return {
    phase: "coder",
    timestamp: new Date().toISOString(),
    input_references: {
