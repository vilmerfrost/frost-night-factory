 ...
 */
export async function runPlannerPhase(idea, research) {
 ...
 * Receives ResearchPhaseJSON, outputs PlannerPhaseJSON
 */
export async function runPlannerPhaseJSON(userPrompt, researchContext, options = {}) {
...
Provide DETAILED output. This plan will drive the entire codebase generation.`;
    ...
    try {
        if (useDeepSeekR1) {
            ...
            rawPlanText = await callAI("PLANNER", plannerPrompt, "You are an expert software architect. Think step-by-step.", undefined, "GENIUS");
        }
        else {
            console.log("⚡ Gemini: Creating architecture plan...");
            rawPlanText = await generateContent(plannerPrompt, "You are a Senior System Architect");
        }
    }
    catch (e) {
 ...
 */
export function legacyToPlannerPhaseJSON(legacy, researchTimestamp) {
    return {
        phase: "planner",
