 ...
 */
export async function runResearchPhaseJSON(
  userPrompt: string,
  options: {
  ...
  if (usePerplexity) {
    try {
      ...
      const perplexityResult = await performDeepResearch(
        isBug 
  ...
  if (useKimi) {
    try {
      ...
      const kimiResult = await callAI(
        "RESEARCH",
        isBug
          ? `Analyze this bug report and provide debugging insights: ${userPrompt}`
          : `Analyze this feature request and provide market insights: ${userPrompt}`,
 ...
 */
export function legacyToResearchPhaseJSON(legacy: ResearchOutput): ResearchPhaseJSON {
  return {
    phase: "research",
