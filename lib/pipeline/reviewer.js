...
export async function runReviewerPhase(repoPath, branchName, originalSpec) {
    // Get git diff
...
Return ONLY valid JSON with this structure:
{
  ...
  "human_todos": [
    ...
    const response = await generateContent(prompt, "You are a Senior Code Reviewer");
    // Extract JSON
...
async function getGitDiff(repoPath, branchName) {
    try {
        // Get diff between main and the branch
        const diff = execSync(`git diff main...${branchName}`, { cwd: repoPath, encoding: "utf-8" });
...
async function getChangedFiles(repoPath, branchName) {
    try {
        const output = execSync(`git diff --name-only main...${branchName}`, { cwd: repoPath, encoding: "utf-8" });
...
async function writeReviewReport(repoPath, branchName, review) {
