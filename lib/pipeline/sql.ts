 ...
 */
export async function runSqlPhaseJSON(
...
Return ONLY SQL code. No markdown code blocks. Start with comments.`;
  ...
  try {
    ...
    rawSqlOutput = await callAI(
      "BACKEND",
      sqlPrompt,
      "You are a Senior PostgreSQL Database Architect specializing in Supabase."
    );
  } catch (e) {
 ...
 */
export function legacyToSqlEditorPhaseJSON(
  legacy: SqlOutput,
  researchTimestamp: string,
  planTimestamp: string,
  coderTimestamp: string
): SqlEditorPhaseJSON {
  // Extract table names from SQL
