 ...
 */
export async function runTesterPhaseJSON(researchContext, planContext, coderContext, sqlContext, repoPath, options = {}) {
    const { runNpmTests = true, runTypeCheck = true, runLinting = true, runSecurityAudit = true } = options;
...
- FINAL RECOMMENDATION: READY_FOR_DEPLOYMENT | NEEDS_FIXES | BLOCKED`;
    ...
    try {
        aiValidationResult = await callAI("AUDIT", validationPrompt, "You are a Senior QA Engineer validating a complete pipeline.");
        rawTestOutput += `\n=== AI VALIDATION ===\n${aiValidationResult}\n`;
    }
    catch (e) {
...
`;
    ...
    const hasBlockers = testResults.some(t => !t.passed && (t.name === "TypeScript Check" || t.name === "Security Audit"));
    if (hasBlockers) {
 ...
 */
export async function runQuickValidation(pipelineContext) {
    const issues = [];
    // Check context chain
 ...
 */
export function generateTraceabilityReport(context) {
    const lines = [
        "═══════════════════════════════════════════════════════════════",
