...
export class QuarantineZone {
     ...
     */
    async validate(id, fileName) {
        const startTime = Date.now();
        const artifact = this.buffer.get(id);
        if (!artifact)
            throw new Error(`Quarantine ID not found: ${id}`);
        const errors = [];
    ...
    // === VALIDATION HELPERS ===
    async checkSyntax(code, fileName) {
        const errors = [];
        try {
    ...
    async checkImports(code, fileName) {
 ...
 */
export async function generateCodeWithQuarantine(generateFn, fileName, maxRetries = 1) {
