...
export class SnapshotManager {
    localSnapshots = new Map();
    /**
     * Take snapshot before risky operations
     */
    async takeSnapshot(pipelineId, phase, workspaceFiles) {
        const snapshot = {
     ...
     */
    async rollback(pipelineId, workspaceRoot) {
        let snapshot = null;
        // Try Supabase first
        if (supabase) {
            try {
     ...
     */
    async markCurrentAsBroken(pipelineId) {
        // Update local
        const localSnapshots = Array.from(this.localSnapshots.values())
     ...
     */
    async cleanup(pipelineId, keepLast = 5) {
