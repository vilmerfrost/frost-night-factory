...
export { 
  QuarantineZone, 
  quarantine, 
  generateCodeWithQuarantine 
} from './quarantine/quarantine-zone';
...
export {
  ...
  type PipelineState,
  type ErrorState,
} from './state-machine/pipeline-state';
...
export {
  ...
  metamorphicValidator,
  validateWithMetamorphic,
} from './validation/metamorphic-validator';
 ...
 */
export async function generateCodeV85(config: GenerationConfig): Promise<GenerationResult> {
 ...
 */
export async function runPipelineStepV85(
  pipelineId: string,
  phase: 'coder' | 'tester' | 'deployer',
  stepFn: () => Promise<void>
 ...
 */
function classifyError(message: string): 'SYNTAX' | 'IMPORT' | 'BUILD' | 'RUNTIME' | 'NETWORK' | 'RATE_LIMIT' {
  const lower = message.toLowerCase();
  
  if (lower.includes('syntax') || lower.includes('parse')) return 'SYNTAX';
  if (lower.includes('import') || lower.includes('module') || lower.includes('cannot find')) return 'IMPORT';
  if (lower.includes('build') || lower.includes('compile') || lower.includes('ts')) return 'BUILD';
  if (lower.includes('network') || lower.includes('econnreset') || lower.includes('timeout')) return 'NETWORK';
